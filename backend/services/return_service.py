...
# ─── FASE I — SEBAB KELUHAN (satu validator, dipakai create & ubah) ─────────
async def _resolve_complaint(code: str, entity_id: str) -> Tuple[str, str, Dict[str, Any]]:
    """Validasi kode keluhan ke MASTER, bukan ke daftar di dalam kode.

    Pemilik boleh menambah kode keluhan lewat layar Master Data, jadi kode yang tidak
    dikenal harus ditolak DENGAN kalimat menuntun — bukan tersimpan diam-diam lalu
    muncul mentah di laporan "keluhan tersering". Kosong tetap boleh: sebab keluhan
    sering baru jelas sesudah barang dilihat (lihat `set_complaint`).

    Satu fungsi dipakai DUA pintu (`create_return` & `set_complaint`) dengan sengaja:
    validator yang disalin akan menyimpang, dan pintu yang lebih longgar selalu yang
    dipakai orang.
    """
    want = str(code or "").strip().lower()
    if not want:
        return "", "", {}
    from services import master_registry as _mr
    meta = await _mr.complaint_reason_meta(want, entity_id)
    if not meta:
        aktif = [r["value"] for r in await _mr.active_complaint_reasons(entity_id)]
        raise ValueError(
            f"Alasan keluhan “{want}” tidak dikenal. Pilihan yang berlaku: "
            f"{', '.join(aktif) or '(belum ada — isi Master Data → Alasan Keluhan Retur)'}.")
    if meta.get("active") is False:
        raise ValueError(
            f"Alasan keluhan “{meta.get('label') or want}” sudah dinonaktifkan "
            f"pemilik — pilih alasan yang masih berlaku.")
    return want, str(meta.get("label") or want), meta


async def set_complaint(return_id: str, code: str, note: str,
                        actor: str) -> Dict[str, Any]:
    """Isi/ubah sebab keluhan pada retur yang sudah ada (ber-jejak audit di router)."""
    ret = await db.sales_returns.find_one({"id": return_id})
    if not ret:
        raise ValueError(f"Return {return_id} tidak ditemukan")
    want, label, meta = await _resolve_complaint(code, ret.get("entity_id", "") or "")
    await db.sales_returns.update_one(
        {"id": return_id},
        {"$set": {"complaint_code": want, "complaint_label": label,
                  "complaint_note": str(note or "").strip(),
                  "complaint_needs_inspection": bool(meta.get("needs_inspection")),
                  "complaint_claimable": bool(meta.get("claimable")),
                  "complaint_set_by": actor, "complaint_set_at": now_iso(),
                  "updated_at": now_iso()}})
    out = await db.sales_returns.find_one({"id": return_id})
    out.pop("_id", None)
    return out
...
async def create_return(
    ...
    # ── FASE I — SEBAB keluhan (master `complaint_reasons`) ───────────────────
    complaint, complaint_label, complaint_meta = await _resolve_complaint(
        complaint_code, entity_id)
    ...
    doc = {
        ...
        # ── FASE I — SEBAB keluhan + PERJALANAN retur (lembar pemilik) ──────
        # Label ikut di-snapshot supaya dokumen tetap terbaca kalau kelak baris
        # master diganti nama — sama seperti snapshot lini & satuan di FASE L/U.
        "complaint_code":  complaint,
        "complaint_label": complaint_label,
        "complaint_note":  str(complaint_note or "").strip(),
        "complaint_needs_inspection": bool(complaint_meta.get("needs_inspection")),
        "complaint_claimable":        bool(complaint_meta.get("claimable")),
        # Milestone: kosong = BELUM terjadi (bukan nol/tanggal palsu). Layar
        # menampilkan garis waktu dari empat field ini + `inspect_done_at`.
        "shipped_to_store_at":    "",
        "shipped_to_customer_at": "",
        "goods_arrived_at":       "",
        "milestones":             [],     # jejak siapa menandai apa, kapan
        # SPK Inspeksi (`inspections`) yang memeriksa retur ini — satu dokumen,
        # ditulis `inspection_service` supaya jejaknya dua arah.
        "inspection_id":     "",
        "inspection_number": "",
        "inspect_done_at":   "",
...
# ─── FASE I — PERJALANAN RETUR (milestone lembar pemilik) ────────────────────
# Lembar kerja pemilik menandai empat saat: SJ Kirim Toko → Kirim ke Cust →
# Barang Sampai → Inspect selesai. Tiga yang pertama ditandai manusia (di sini),
# yang keempat DITURUNKAN dari dokumen inspeksi (`attach_inspection`/`mark_inspected`)
# supaya tidak ada dua tanggal "inspeksi selesai" yang bisa berbeda.
RETURN_MILESTONES: Dict[str, Tuple[str, str]] = {
    "shipped_to_store":    ("shipped_to_store_at",    "SJ Kirim Toko"),
    "shipped_to_customer": ("shipped_to_customer_at", "Kirim ke pelanggan"),
    "goods_arrived":       ("goods_arrived_at",       "Barang sampai"),
}


async def mark_milestone(return_id: str, milestone: str, actor: str,
                         at: str = "", note: str = "") -> Dict[str, Any]:
    """Tandai satu saat perjalanan retur. Idempotent-aman & selalu ber-jejak.

    Aturan yang dijaga di sini (bukan di layar):
      * kode milestone harus dikenal — salah ketik tidak boleh menciptakan field baru
        yang tak pernah dibaca layar mana pun;
      * **tanggal masa depan ditolak** — penanda perjalanan adalah catatan, bukan
        rencana. Ini yang membuat "Barang Sampai" bisa dipercaya sebagai bukti;
      * "Barang sampai" menuntut salah satu pengiriman sudah ditandai — barang tidak
        bisa sampai sebelum pernah dikirim, dan urutan itulah yang membuat garis
        waktu bisa dibaca sebagai cerita;
      * menandai ULANG diperbolehkan (koreksi salah tanggal itu wajar) tetapi
        tanggal lama TIDAK hilang: setiap penandaan menambah baris `milestones[]`.
    """
    key = str(milestone or "").strip().lower()
    if key not in RETURN_MILESTONES:
        raise ValueError(
            f"Penanda perjalanan “{milestone}” tidak dikenal. Pilihan: "
            f"{', '.join(RETURN_MILESTONES)}.")
    ret = await db.sales_returns.find_one({"id": return_id})
    if not ret:
        raise ValueError(f"Return {return_id} tidak ditemukan")

    field, label = RETURN_MILESTONES[key]
    stamp = str(at or "").strip() or now_iso()
    if stamp[:10] > now_iso()[:10]:
        raise ValueError(
            f"Tanggal “{label}” tidak boleh di masa depan — penanda perjalanan adalah "
            f"catatan kejadian, bukan rencana.")
    if key == "goods_arrived" and not (ret.get("shipped_to_store_at")
                                       or ret.get("shipped_to_customer_at")):
        raise ValueError(
            "Tandai pengirimannya dulu (SJ Kirim Toko atau Kirim ke pelanggan) — "
            "barang tidak bisa “sampai” sebelum pernah dikirim.")

    entry = {"id": new_id("rmst"), "milestone": key, "label": label, "at": stamp,
             "by": actor, "note": str(note or "").strip(),
             "replaced": str(ret.get(field) or ""), "recorded_at": now_iso()}
    await db.sales_returns.update_one(
        {"id": return_id},
        {"$set": {field: stamp, "updated_at": now_iso()},
...
async def attach_inspection(return_id: str, inspection_id: str,
                            inspection_number: str) -> None:
    """Catat SPK Inspeksi yang memeriksa retur ini (dipanggil `inspection_service`).

    Ditulis dari SATU sisi saja (service inspeksi) supaya tidak ada dua penulis untuk
    fakta yang sama. Jejak dokumen dua arah tetap ada lewat `doc_refs_service`.
    """
    if not (return_id and inspection_id):
        return
    await db.sales_returns.update_one(
        {"id": return_id},
...
async def mark_inspected_by_document(return_id: str, at: str = "") -> None:
    """`inspect_done_at` DITURUNKAN dari dokumen inspeksi yang ditutup.

    Sengaja tidak ada endpoint manual untuk field ini: dua tanggal "inspeksi selesai"
    yang bisa berbeda adalah kelas bug yang paling sering menipu di repo ini.
    """
    if not return_id:
        return
    await db.sales_returns.update_one(
        {"id": return_id},
        {"$set": {"inspect_done_at": str(at or "") or now_iso(),
                  "updated_at": now_iso()}})
